...
function App() {
  ...
  async function loadSessions() {
    try {
      let res = await fetch("/api/sessions", { headers: { Authorization: "Bearer " + getToken() } });
  ...
  async function createNewChat() {
    try {
      let res = await fetch("/api/sessions", { method: "POST", headers: { Authorization: "Bearer " + getToken() } });
  ...
  async function selectSession(id) {
    ...
    try {
      let res = await fetch(`/api/sessions/${id}/messages`, { headers: { Authorization: "Bearer " + getToken() } });
      let data = await res.json();
      if (data.length === 0) {
        setMessages([{ role: "bot", content: `Hi ${username}! How can I help you today?`, time: getTime() }]);
      } else {
  ...
  async function checkRagStatus(sessionId) {
    try {
      let res = await fetch(`/api/document/status?sessionId=${sessionId}`, { headers: { Authorization: "Bearer " + getToken() } });
      let data = await res.json();
  ...
  async function handleFileChange(e) {
    ...
    if (!validTypes.some(ext => file.name.toLowerCase().endsWith(ext))) { alert("Only PDF, PNG, JPG files are supported."); return; }
    const isPdf = file.name.toLowerCase().endsWith(".pdf");
    const maxSize = isPdf ? 10 * 1024 * 1024 : 5 * 1024 * 1024;
    if (file.size > maxSize) { alert(isPdf ? "PDF must be under 10MB." : "Image must be under 5MB."); return; }
    setIsUploading(true);
    const formData = new FormData();
    formData.append("file", file);
    formData.append("sessionId", currentSessionId);
    try {
      let res = await fetch("/api/document/upload", { method: "POST", headers: { Authorization: "Bearer " + getToken() }, body: formData });
      let data = await res.json();
      if (data.success) {
        setRagActive(true);
        setRagFileName(file.name);
        setMessages((prev) => [...prev, { role: "bot", content: `📄 **${file.name}** uploaded successfully! ${data.message} You can now ask questions about this document.`, time: getTime() }]);
  ...
  async function handleClearDoc() {
    ...
    try {
      await fetch(`/api/document/clear?sessionId=${currentSessionId}`, { method: "DELETE", headers: { Authorization: "Bearer " + getToken() } });
      setRagActive(false);
      setRagFileName("");
      setMessages((prev) => [...prev, { role: "bot", content: "📄 Document removed. Back to normal chat mode.", time: getTime() }]);
    } catch (err) { console.error(err); }
  }
  ...
  async function deleteSession(id, e) {
    ...
    try {
      await fetch(`/api/sessions/${id}`, { method: "DELETE", headers: { Authorization: "Bearer " + getToken() } });
      let updated = sessions.filter((s) => s.id !== id);
      setSessions(updated);
      if (id === currentSessionId) { if (updated.length > 0) selectSession(updated[0].id); else await createNewChat(); }
    } catch (err) { console.error(err); }
  }

  function startRename(session, e) { e.stopPropagation(); setEditingSessionId(session.id); setEditTitle(session.title); }

  async function saveRename(id) {
    if (!editTitle.trim()) { setEditingSessionId(null); return; }
    try {
      await fetch(`/api/sessions/${id}`, { method: "PUT", headers: { "Content-Type": "application/json", Authorization: "Bearer " + getToken() }, body: JSON.stringify({ title: editTitle }) });
  ...
  function logout() {
    ...
    localStorage.removeItem("name"); localStorage.removeItem("role");
  ...
  async function ask() {
    ...
    try {
      ...
      // ✅ TOKEN LIMIT — show orange warning with countdown
      if (res.status === 429) {
        let data = await res.json();
        setMessages([...newMessages, {
          role: "bot",
      ...
      let data = await res.json();
      setMessages([...newMessages, { role: "bot", content: data.answer, time: getTime(), ragUsed: data.ragUsed }]);
      setSessions((prev) => prev.map((s) => (s.id === currentSessionId ? { ...s, title: data.sessionTitle } : s)));

    } catch (err) {
      if (err.name === "AbortError") return;
      setMessages([...newMessages, { role: "bot", content: "Could not reach server. Please try again.", time: getTime(), error: true }]);
    }
      ...
      <div className="layout">
        <motion.div className="sidebar" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
          ...
          <div className="sessionList">
            <AnimatePresence>
              {sessions.map((s) => (
                <motion.div key={s.id} className={`sessionItem ${s.id === currentSessionId ? "active" : ""}`}
                  onClick={() => selectSession(s.id)} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                  {editingSessionId === s.id ? (
                    <input className="renameInput" value={editTitle} onChange={(e) => setEditTitle(e.target.value)}
                      onBlur={() => saveRename(s.id)} onKeyDown={(e) => e.key === "Enter" && saveRename(s.id)}
          ...
          <div className="sidebarFooter">
            ...
            {role === "ADMIN" && (<button className="sidebarBtn" onClick={() => (window.location.href = "/users.html")}>Manage Users</button>)}
        ...
        <div className="app">
          ...
          <div className="chatbox" ref={chatboxRef}>
            <AnimatePresence initial={false}>
              {messages.map((m, i) => (
                <motion.div key={i} className={`messageRow ${m.role}`}
                  initial={{ opacity: 0, y: 14, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }}
                  transition={{ duration: 0.28, ease: "easeOut" }}>
                  {m.role === "bot" && <div className="avatar bot-avatar">AI</div>}

                  <div className={`message ${m.role} ${m.tokenLimit ? "tokenLimit" : m.error ? "error" : ""}`}>
                    <div className="messageText">
                      {m.tokenLimit ? (
                        <span>
                          <span className="tokenLimitIcon">⏳</span>
                          {m.content}
                          <br/>
                          <span className="tokenLimitTimer">🔄 Resets in {m.minutesUntilReset} minute(s)</span>
                        </span>
                      ) : m.role === "bot" ? (
                    ...
                    <div className="messageFooter">
                      ...
                      {m.role === "bot" && !m.error && !m.tokenLimit && (
                        <button id={`copy-${i}`} className="copyBtn" onClick={() => copyMessage(m.content, i)}>Copy</button>
                      )}
                    </div>
                  </div>

                  {m.role === "user" && (<div className="avatar user-avatar">{username.charAt(0).toUpperCase()}</div>)}
                </motion.div>
              ))}
              {isTyping && (
